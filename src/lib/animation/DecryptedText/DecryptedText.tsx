...
export default function DecryptedText({
  ...
  useEffect(() => {
    ...
    const availableChars = useOriginalCharsOnly ? [...new Set(text)].filter((char) => char !== " ") : [...characters];

    const shuffleText = (originalText: string, currentRevealed: Set<number>): string => {
      if (useOriginalCharsOnly) {
        const positions = [...originalText].map((char, index) => ({
          char,
          isSpace: char === " ",
          index: index,
        ...
        for (let index = nonSpaceChars.length - 1; index > 0; index--) {
          ...
          [nonSpaceChars[index], nonSpaceChars[index_]] = [nonSpaceChars[index_], nonSpaceChars[index]];
        }
